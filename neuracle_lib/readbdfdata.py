...
def readbdfdata(filename, pathname):
    ...
    if 'edf' in filename[0]:  ## DSI
        ...
        # 从raw对象中获取所有通道的数据，除了最后一个通道。
        # 这是因为最后一个通道可能包含一些额外的信息，而不是实际的EEG数据。
        # 将提取的数据赋值给data变量，而忽略第二个返回值（通常是一个时间戳数组）。
        data, _ = raw[:-1]
        ...
        ch_names = raw.info['ch_names']
        # 从 raw 对象中提取所有通道的数据，但不包括最后一个通道
        # 将提取的数据赋值给data变量，而忽略第二个返回值（通常是时间戳数组）。
